...
try:
    ...
    ML_AVAILABLE = False
    _logger.warning("Prema ML: scikit-learn/joblib not installed. Run: pip install scikit-learn joblib --break-system-packages")
...
# Input:  vendor name + line description
# Output: account code prediction
# ─────────────────────────────────────────────

def train_account_classifier(env):
    """Train model from historical bill lines."""
    if not ML_AVAILABLE:
    ...
    try:
        ...
        joblib.dump({"pipeline": pipeline, "classes": list(set(y))}, path)
...
def predict_account(vendor_name, description):
    """Predict account code for a bill line."""
    if not ML_AVAILABLE:
        return None
    path = _model_path("account_classifier")
...
# Input:  vendor country + amount + description keywords
# Output: tax code prediction (GST, HST, zero-rated, etc.)
# ─────────────────────────────────────────────
...
def train_tax_predictor(env):
    ...
    try:
        moves = env["account.move"].search_read(
            ...
            ["id", "partner_id", "amount_total", "invoice_line_ids", "currency_id"],
            limit=3000,
        )
        ...
        for move in moves:
            ...
            for line in lines:
                ...
                features = "{} {} {} {}".format(vendor, currency, desc, "large" if amount > 1000 else "small")
                X.append(features.lower())
                y.append(tax_names[0])  # primary tax
...
def predict_tax(vendor_name, description, currency="CAD", amount=0):
    if not ML_AVAILABLE:
        return None
...
# Uses bill fingerprinting (amount + vendor + ref hash)
# Returns similarity score, no training needed
# ─────────────────────────────────────────────
...
def check_duplicate_fingerprint(env, partner_name, ref, amount, exclude_id=None):
    """Check if a bill with similar fingerprint exists."""
    if not ref:
        return None
    ...
    existing = env["account.move"].search_read(
        ...
        ["id", "name", "partner_id", "ref", "amount_total"],
        limit=200,
    )
...
# ─────────────────────────────────────────────
# 4. FIELD CORRECTOR (learns from user edits)
# Input:  AI-extracted field values
# Output: corrected field values based on past corrections
# ─────────────────────────────────────────────

def train_field_corrector(correction_records):
    """
    Train from correction records:
    correction_records = list of {"input": {...}, "output": {...}}
    Trains separate models per field.
    """
    if not ML_AVAILABLE:
        return {"error": "scikit-learn not installed"}
...
def predict_field_correction(field, ai_value):
    """Given an AI-extracted value, predict what the user would change it to."""

...
def scan(env):
    ...
    try:
        moves = env["account.move"].search_read(
            [("state", "=", "posted")], ["id", "name", "date"], limit=500)
        for move in moves:
            lines = env["account.move.line"].search_read(
                [("move_id", "=", move["id"])], ["debit", "credit"])
            diff = round(abs(sum(l["debit"] for l in lines) - sum(l["credit"] for l in lines)), 2)
            if diff > 0.01:
                issues.append({
                    "category": "journals", "code": "JNL_IMBALANCED", "risk": "critical",
                    "title": "Imbalanced Entry: {}".format(move["name"]),
                    "detail": "Entry ID {} is posted but has debit/credit difference of {}.".format(
                        move["id"], diff),
    ...
    try:
        ...
        for move in old:
            issues.append({
                "category": "journals", "code": "JNL_OLD_DRAFT", "risk": "medium",
                "title": "Old Draft Entry: {}".format(move["name"] or "Unnamed"),
                "detail": "Entry ID {} in draft since {} (>30 days). Type: {}.".format(
                    move["id"], move["date"], move["move_type"]),
                "affected_model": "account.move",
                "affected_ids": [move["id"]],
                "fix_action": None,
            })
    ...
    try:
        orphans = env["account.move.line"].search_read(
            [("account_id", "=", False)], ["id", "move_id"], limit=50)
        for line in orphans:
            issues.append({
                "category": "journals", "code": "JNL_ORPHAN_LINE", "risk": "high",
                "title": "Journal Line Without Account",
                "detail": "Line ID {} on move '{}' has no account.".format(
                    line["id"], line["move_id"][1] if line["move_id"] else "?"),
    ...
    try:
        empty = env["account.move"].search_read(
            [("line_ids", "=", False), ("state", "=", "draft")],
            ["id", "name", "date"], limit=50)
        for move in empty:
            issues.append({
                "category": "journals", "code": "JNL_EMPTY_ENTRY", "risk": "medium",
                "title": "Empty Journal Entry: {}".format(move["name"] or "Unnamed"),
                "detail": "Entry ID {} has no lines. Date: {}.".format(move["id"], move["date"]),
                "affected_model": "account.move",
                "affected_ids": [move["id"]],
                "fix_action": {
                    "type": "delete_record", "model": "account.move",
                    "record_id": move["id"],

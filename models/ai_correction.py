...
after every 10 new corrections.
"""
import json
import logging
...
class PremaAICorrection(models.Model):
    ...
    def record_correction(self, context_type, ai_suggestion, user_correction, tags=""):
        ...
        record = self.create({
            ...
            "user_correction": json.dumps(user_correction, default=str) if not isinstance(user_correction, str) else user_correction,
            "lesson": lesson,
            "tags": tags,
        })
    ...
    def get_relevant_lessons(self, context_type=None, tags=None, limit=10):
        ...
        corrections = self.search(domain, order="applied_count desc, create_date desc", limit=limit)
        return [{"lesson": c.lesson, "context": c.context_type, "count": c.applied_count} for c in corrections]

    @api.model
    def build_memory_prompt(self, context_type=None, tags=None):
        lessons = self.get_relevant_lessons(context_type=context_type, tags=tags, limit=8)
        if not lessons:
            return ""
...
def _generate_lesson(context_type, ai_suggestion, user_correction):
    try:
        if context_type == "bill_creation":
            ...
            return "Fix rejected on '{}' ({}): {}".format(fix.get("model", "?"), fix.get("description", ""), reason)
        elif context_type == "fix_modification":
            ai = ai_suggestion if isinstance(ai_suggestion, dict) else json.loads(ai_suggestion or "{}")
            uc = user_correction if isinstance(user_correction, dict) else json.loads(user_correction or "{}")
            return "Fix modified on '{}': {} → {}".format(
                ai.get("model", "?"), json.dumps(ai.get("values", {})), json.dumps(uc.get("values", {})))
    except Exception:
        pass

...
def scan(env):
    ...
    for acc in accounts:
        ...
        if key in seen:
            issues.append({
                "category": "accounts", "code": "ACC_DUPLICATE_CODE", "risk": "high",
                "title": "Duplicate Account Code: {}".format(acc["code"]),
                "detail": "Account '{}' (ID {}) shares code '{}' with ID {}.".format(
                    acc["name"], acc["id"], acc["code"], seen[key]),
                "affected_model": "account.account",
                "affected_ids": [acc["id"], seen[key]],
                "fix_action": None,
            })
        else:
            seen[key] = acc["id"]
    ...
    try:
        ...
        for line in lines:
            issues.append({
                "category": "accounts", "code": "ACC_WRONG_JOURNAL_TYPE", "risk": "medium",

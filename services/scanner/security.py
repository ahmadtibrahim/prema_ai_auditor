...
def scan(env):
    ...
    # 1. Admin user review
    try:
        admin = env["res.users"].sudo().search_read(
            [("id", "=", 2)], ["name", "login"])
        if admin:
            issues.append({
                "category": "security", "code": "SEC_ADMIN_REVIEW", "risk": "medium",
    ...
    try:
        ...
        if inactive:
            issues.append({
                "category": "security", "code": "SEC_INACTIVE_USERS", "risk": "low",
                ...
                "affected_model": "res.users",
                "affected_ids": [u["id"] for u in inactive],
                "fix_action": None,
            })
    except Exception:
        pass
    ...
    try:
        ...
        if internal_group:
            portal = env["res.users"].sudo().search_read(
                ...
                ["id", "name", "login", "groups_id"], limit=100)
            for user in portal:
                if internal_group.id in (user.get("groups_id") or []):
                    issues.append({
                        "category": "security", "code": "SEC_PORTAL_INTERNAL_ACCESS",
                        "risk": "critical",
                        "title": "Portal User Has Internal Access: {}".format(user["login"]),
                        "detail": "Portal user '{}' (ID {}) is in the internal users group.".format(
                            user["name"], user["id"]),
                        "affected_model": "res.users",
                        "affected_ids": [user["id"]],
                        "fix_action": None,
                    })
    except Exception:
        pass
    ...
    try:
        ...
        if admin_group:
            ...
            if len(admins) > 3:
                issues.append({
                    "category": "security", "code": "SEC_TOO_MANY_ADMINS", "risk": "medium",
                    ...
                    "affected_model": "res.users",
                    "affected_ids": [u["id"] for u in admins],
                    "fix_action": None,
                })
    except Exception:
        pass
